...
/**
 * Container for the WolfTasks project that provides the user the ability to
 * interact with notebooks, task lists, and tasks.
 * 
...
public class WolfTasksGUI extends JFrame implements ActionListener {
	...
	/**
	 * Constructs a ServiceWolfGUI object that will contain a JMenuBar and a JPanel
		...
		if (e.getSource() == itemNew) {
			if (notebook != null && notebook.isChanged()) {
				int select = JOptionPane.showConfirmDialog(null,
						"Current Notebook is unsaved. Would you like to save before creating a new Notebook?",
	...
	private class TaskListPanel extends JPanel implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			if (e.getSource() == comboTaskLists) {
				...
				try {
					String taskListName = (String) JOptionPane.showInputDialog(this, "Task List Name?",
							"Create New Task List", JOptionPane.QUESTION_MESSAGE);
				...
				try {
					if (notebook.getCurrentTaskList() instanceof ActiveTaskList) {
						...
						String taskListName = (String) JOptionPane.showInputDialog(this, "Update Task List Name Name?",
								"Edit Task List", JOptionPane.QUESTION_MESSAGE, null, null,
								notebook.getCurrentTaskList().getTaskListName());
				...
				try {
					if (notebook.getCurrentTaskList() instanceof ActiveTaskList) {
						...
						int selection = JOptionPane.showOptionDialog(this,
								"Are you sure you want to delete " + notebook.getCurrentTaskList().getTaskListName()
										+ "?",
								"Delete Task List", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE, null, null,
								null);
						if (selection == 0) { // Yes
	...
	private class TaskPanel extends JPanel implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (e.getSource() == btnAddEdit) {
				try {
					if (idx == -1) {
						Task t = new Task(txtTaskName.getText(), txtDescription.getText(), checkRecurring.isSelected(),
								checkActive.isSelected());
						notebook.addTask(t);
					} else {
						notebook.editTask(idx, txtTaskName.getText(), txtDescription.getText(),
								checkRecurring.isSelected(), checkActive.isSelected());
					}
					setTask(-1);
				} catch (IllegalArgumentException iae) {
					JOptionPane.showMessageDialog(WolfTasksGUI.this, iae.getMessage());
